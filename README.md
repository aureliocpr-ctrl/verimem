...
**What it costs on disk, and how long it takes.** Measured, not estimated — and
worth knowing before you start, because most of it is not us:

| | size | time |
|---|---|---|
| `pip install verimem` — `torch` alone is about half | **~1.0–1.2 GB** | 466 s (Windows) · 1315 s (WSL) |
| first `verimem warmup` — embedding, reranker, gate models | **~2.3 GB** | the judge model is 746 MB of it |
| **total, first run on a clean machine** | **~3.3 GB** | |

`torch` is the single biggest item (~527 MB) and it comes from the dependency
tree, not from Verimem's own code. Sizes are decimal MB (10⁶): the judge model
directory is 746 058 368 bytes, which a tool reporting MiB (2²⁰) shows as 711 —
same bytes, different unit.

> ⚠️ **Without the judge model the write gate has no judge.** `verimem warmup`
> downloads it once; `verimem doctor` tells you whether it is there. If it is
> missing, writes are still stored — but a claim its source does not support is
> not held back, and the guarantee at the top of this page does not apply. Check
> with `verimem doctor` before trusting a store you just created.
