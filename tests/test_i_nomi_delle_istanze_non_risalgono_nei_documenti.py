...
#:   11  su `2714029e`, il 13/09 alle 13:20, dopo il merge della pulizia:
#:        nella sola `docs/stato-reale/` sono passati da 81 a 0. Gli 11 che
#:        restano stanno tutti FUORI da `stato-reale/` — 10 in `ricerca/`, 1 in
#:        `mondo-esterno/` — ed erano fuori dal perimetro di quella pulizia.
#:
#: ⚠️⚠️ NON E' UN TETTO ALZATO: il 6 non ha mai misurato questa popolazione.
#: Il 12/09 l'ho contato con `git ls-files 'docs/**/*.md'`, che si ferma a un
#: livello; questo test usa `rglob`, che scende in tutto l'albero — **due
#: popolazioni diverse chiamate con lo stesso nome**, e la CI l'ha detto subito
#: («sono 11, il tetto e' 6») su ogni gamba della matrice. La regola che avevo
#: gia' scritto e violato e': **un numero si fissa SOLO dove lo si misura**, cioe'
#: con il criterio del test, non con quello comodo alla riga di comando.
TETTO = 11
