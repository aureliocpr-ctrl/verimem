...
Bug found by critic-orchestrator counterexample worker (confidence 0.93):
  ...
  `real.__spec__` (name='verimem.X'). CPython's _bootstrap._load_unlocked
  ignores `sys.modules[fullname]` set by the finder and re-executes the
  module body of verimem.X, creating a SECOND copy of the module under
  the hippoagent.X alias.

Effects:
  - hippoagent.skill.Skill is NOT verimem.skill.Skill (two distinct classes)
...
_load_unlocked does not re-execute verimem.X — the cached module is reused.
"""
...
def test_identity_when_engram_imported_first():
    """Critic counterexample: verimem.skill loaded BEFORE hippoagent.skill must
    still expose the SAME class via both import paths."""
    rc, out, err = _run_isolated("""
import verimem.skill           # loads verimem.skill first
from hippoagent.skill import Skill as ShimSkill
import verimem.skill as engram_skill_mod
print('verimem.skill is hippoagent.skill?', engram_skill_mod.Skill is ShimSkill)
""")
...
def test_isinstance_cross_import_path():
    """Instance built via verimem.skill must satisfy isinstance(x, hippoagent.skill.Skill)."""
    rc, out, err = _run_isolated("""
from verimem.skill import Skill as EngramSkill
...
def test_module_singleton_state_not_duplicated():
    """Module-level state set in verimem.X must be visible via hippoagent.X."""
    rc, out, err = _run_isolated("""
import verimem.skill as eng
...
""")
    ...
    assert out.endswith("set_via_engram"), (
        f"Module state not shared between verimem.skill and hippoagent.skill:\n{out}\nstderr: {err}"
    )


def test_sys_modules_keys_share_object_identity():
    """sys.modules['verimem.X'] and sys.modules['hippoagent.X'] must be the same object."""
    rc, out, err = _run_isolated("""
import verimem.skill
from hippoagent.skill import Skill
import sys
same = sys.modules.get('verimem.skill') is sys.modules.get('hippoagent.skill')
