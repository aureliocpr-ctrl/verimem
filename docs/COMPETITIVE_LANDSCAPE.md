...
- **NOTE (corrected 2026-07-25):** this line used to read "Engram's PPR core already equals/beats the HippoRAG reference; the gaps are seed-weighting + recognition-gating, not the core." **That was false in production.** Measured on the real corpus: the graph held 8005 entities / 24953 fact links / 81477 edges and `ppr_seeded_fact_ids` returned **0 ids in 0.0 ms on every real query** — the PPR core was not beating anything, it was not running. Cause: `extract_entities_lite` is ingest-tuned and has no pattern for a single lowercase word, and this corpus's proper nouns are lowercase (`verimem`, `engram`). Fixed in `ef2cc87c`+`28d80775` (query tokens are now resolved against the store): graph signal 3/10 → 10/10 queries. **Still a real gap vs the reference:** HippoRAG-2 links query entities to KG nodes with a *retrieval encoder* (embedding similarity), so it resolves a paraphrase; `get_by_name` is an exact normalized match, so it does not. Seed-weighting and recognition-gating remain open on top of that.
