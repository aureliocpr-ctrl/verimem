...
| `ENGRAM_ADMISSION_GATE` (L0 telemetry/dup/injection routing) | **ON** (default since 0.7.0) | "Every write passes an admission gate" (headline, line 3) | ✅ *(re-measured 2026-07-27: `gate_enabled()` → True with no env set; the ⚠️ below was true before 0.7.0)* |
| L3 contradiction (`validate=full`) | **ON** (preset `balanced` is `validate: full`) | "or **contradictory** assertions are flagged" (line 20-22) | ✅ *(re-measured 2026-07-27: both `strict` and `balanced` ship `validate: 'full'`; the ❌ below predates the flip)* |
| `ENGRAM_GROUNDING_WRITE` / preset `ground` | **ON** (preset `balanced` is `ground: True`, since 2026-07-17) | "must be backed by evidence to gain status"; site's AUROC 0.971 moat | ✅ *(re-measured 2026-07-27: the moat is ON out-of-the-box; the ❌ below described the pre-2026-07-17 default)* |
...
| Provenance signing (`VERIMEM_PROVENANCE_KEY`, legacy `ENGRAM_PROVENANCE_KEY`) | **OFF by default, and now genuinely opt-in** (wired `531599bc`) | README says "*(opt-in)*" | ✅ *since `531599bc`.* Until 2026-07-27 this row read "honestly labeled" and was **not**: the crypto was sound (bench 9/9 — HMAC binds ref-body to proposition, cross-fact replay rejected, tampered body/signature/key rejected, `actor:` exempt, `canonical_source` unaffected by the `#sig=` tail) but an exhaustive repo grep found `provenance_signing` ONLY in `CHANGELOG.md:788` and its own tests, so **setting the key was inert — there was no opt-in to take**. `store()` now consults `signature_offenders()`: with no key the layer stays inert, a valid signature is admitted, an UNSIGNED ref is untouched (historical provenance is unsigned), and only a ref CLAIMING a signature that does not verify is quarantined. TDD falsified (2 tests red before the wiring, red again when the pre-fix files are restored), full suite 8190 green |
