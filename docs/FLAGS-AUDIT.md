...
| Provenance signing (`ENGRAM_PROVENANCE_KEY`) | **NOT WIRED** — not merely off | README says "*(opt-in)*" | ❌ **there is no opt-in to take**: setting the key is inert. Bench 2026-07-27 proves the crypto itself is sound (9/9: HMAC binds ref-body to proposition, cross-fact replay rejected, tampered body/signature/key rejected, `actor:` exempt, `canonical_source` unaffected by the `#sig=` tail) — but an exhaustive repo grep finds `provenance_signing` ONLY in `CHANGELOG.md:788` and its own tests. No write or read path ever calls `sign_ref` / `verify_ref` / `verify_fact_refs` / `provenance_key`. Either wire it into the write path or drop it from the shipped surface; "opt-in" is not one of the honest options |
