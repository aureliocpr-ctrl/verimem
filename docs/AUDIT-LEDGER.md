...
## Addendum — moat E2E col judge opus (2026-07-17 ~06:00, `a88f081`)
`moat_e2e_opus.py` (n=12) prova il PRODOTTO, non il judge isolato: `Memory(llm=opus).add(fact, source=)` col moat ON di default. **Confab quarantinati 12/12 (100%)**, faithful ammessi 11/12. L'unica falsa-quarantena — «The bridge was completed in 1998» — NON veniva dal grounding (L4 impeccabile: 12/12 confab, 0 faithful falsati, score 100 sul ponte) ma dal detector lessicale **L1.13** che scattava sulla parola "completed". Fix nell'arco di escalation (`anti_confab_gate`): un verbo di completamento/creazione **passivo ancorato a un anno di calendario, senza dev-artifact** = fatto-mondo storico → soppresso come per i fatti personali (advisory, resta richiamabile); i claim dev-ancorati («The migration was completed in 2023») e l'auto-narrazione dell'agente («task done», «I finished the task») continuano a escalare. TDD `test_gate_historical_completion.py` (3, red→green) + regression 38 gate/completion + net 1026 `-k gate|confab|grounding|l1|moat`. E2E ri-verificato: il ponte ora `model_claim`/grounding 100 → moat effettivo **12/12 + 12/12**. Soglia write-gate **resta 40**: il judge separa netto 0/100 (il 40-vs-50 era un limite del CE su SNLI, non del path judge di default).
