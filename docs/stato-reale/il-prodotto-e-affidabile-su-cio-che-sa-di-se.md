...
| ④ | «a session NOTE skips that screen … **the moat is UNAFFECTED: with a source**» — `agent_guide` | screen saltato **2/2**, moat gira **2/2** | 🟢 **come meccanismo** | 🔴 **la condizione è rara**: `W7-69` di *Paragone* misura **65% del corpus** che salta `L1`, e sono checkpoint → tipicamente **senza fonte** |
...
Poi *Paragone* ha misurato che il **65% del corpus** salta quello screen, e sono
checkpoint di sessione — cioè scritture che una fonte tipicamente **non** ce
l'hanno.
...
- **Non assolve i miei banchi.** Oggi ho contato **nove difetti nel mio
  ...
  vicenda per `same-source`, uno strumento che interroga un'altra tabella.
