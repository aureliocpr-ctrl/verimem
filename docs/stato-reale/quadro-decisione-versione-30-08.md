...
## 04/09 19:10 — la 0.7.6 è taggata e pubblicata

- **Tag** `v0.7.6` (oggetto `09bc283a`) su `049114259e5123516d76eec8148b5a9ab6f2646b`, spinto alle 19:08 dal lead dall'albero condiviso; il tag locale del 24/08 (`397c6375`, mai su origin) cancellato prima.
- **Cancelli** (`scripts/cancelli_del_tag.py`, versione di ws8 che legge il commit e non l'albero, sha espanso con `git rev-parse`): 11/11 OK, EXIT=0.
- **Smoke da utente vero, stesso wheel** (sha256 `a99f64bc…4ee7`, artefatto del run `33802751438`): Windows 2026-09-04 18:55 EXIT=0 9/9 (ws8) · WSL 2026-09-04 18:45 EXIT=0 9/9 (lead). I tre passi che provano le cure: fonte giudicata (99,356), claim smentito fermato (1,063, `quarantined`), `server.version=0.7.6`.
- **Publish**: run `33898909009`, `completed/success` (gate «la CI del commit taggato» + `build-and-publish`). Verifica dal servito: `pip download verimem==0.7.6` e smoke WSL sul wheel servito da PyPI, con lo sha256 confrontato con l'artefatto — esito nel registro dello smoke.
- **Cosa resta dichiarato e aperto** (nel README e nel CHANGELOG del pacchetto): il giudice ribaltato da una frase estranea accanto a una contraddizione (limite «Length», numeri più forti di ieri sera da aggiungere al README nel prossimo pezzo docs); i debiti LANT-174 (segfault sporadico di `test_hang_watchdog`; L4.1 che legge le cifre di un identificatore, ricomparso oggi su «04/09» e «11 OK»); `as_of` accettato e ignorato da due porte (voto aperto); la porta MCP che non dichiara gli scaduti.
- **Regime dal tag** (post `4dad03888aef5073`): un pezzo alla volta su main, poi 24 ore di esercizio senza cure nuove.
