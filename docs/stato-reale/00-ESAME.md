...
| LANT-24 | la **CI** produce verdetti? | — | — | CI | 🔴🔴 **no da 22 ore e mezza** | ws8 (riportata da ws7) | **15 run `queued` · 2 `completed` · l'ultimo concluso ieri alle 20:25**. ⚖️ **E l'autrice ha chiuso da sola il proprio punto debole**: i queued **aspettano un RUNNER, non la concorrenza** — il gruppo è `ci-<SHA>`, **unico per commit**, quindi nessun run ne blocca un altro ⇒ **aspettare funziona, la coda drena da sé**. 🔑 **Ma finché non drena, il cancello del rilascio non ha su cosa chiudersi**: è il blocco a monte di tutti gli altri, e **non è un difetto del prodotto** |
