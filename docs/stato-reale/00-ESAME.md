...
| 31 | il giudice riconosce che una fonte **implica** il fatto, o pretende che lo **citi**? | — | EN | CLI | 🔴 **premia il ricalco, punisce l'interpretazione** | ws1 | A/B a variabile singola, stessa `--source`: «*the cold pip install took 397 seconds*» → **5.54, quarantinato** · «*the log line reads `install_secondi=397`*» → **99.95, ammesso**. Seconda coppia: **0.64 → 99.48**. 🚨 **È un incentivo, non solo un errore: chi impara come passa il gate scrive citazioni invece di conoscenza — la cura peggiora la memoria** |
| 32 | **quanto** costa parafrasare? (controllo indipendente sulla riga 31) | — | IT | CLI | 🟡 **1,2 punti, non 94** | ws7 | A/B a variabile singola sulla **stessa** source (una riga di `git log`): parafrasi «*è stato creato il 2026-07-21 alle 15:23:21*» → **97,87 ammesso** · citazione «*la riga legge 2026-07-21 15:23:21 +0200*» → **99,10 ammesso**. ⇒ **L'effetto della riga 31 è reale ma NON si riproduce qui**: la parafrasi da sola costa poco. 🔑 **Candidato per la differenza: nel caso di ws1 la fonte è in forma MACCHINA (`chiave=valore`) e il claim in prosa** — non «parafrasi», ma **traduzione fra due registri**. Non misurato: chi lo prova lo scriva qui |
