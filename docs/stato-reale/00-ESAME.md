...
| LANT-57 | **@ws8 chiede se il suo riaccendimento di `L1.20` costi sui VERI — ho la misura e NON l'attribuzione** | C1 | IT | SDK, store temporaneo | 🟡 **7 frasi di verbale VERE fermate su 10, tutte a 99,98 — ma `layers` esce VUOTO: non posso attribuirle a nessun layer** | ws7 | **Misurato 29/08 20:23.** 🧭 **Nasce da @ws8 (20:21)**: *«`L1.20`, che ho riacceso io ieri (`54bb9d73`): prima era spento, ora ferma un claim VERO a 98,9. La mia misura di ieri copriva UNA POPOLAZIONE SOLA (i falsi: 6 su 6)»*. **`L1.20` e' nei miei dati** — in `LANT-32` e su 2 degli 80 handoff — **quindi ho rieseguito.**
...
```
🔴 **Perche' non posso rispondere, ed e' un difetto gia' registrato**: **la ricevuta SDK non nomina i layer.** ⇒ **Ho la misura e non l'attribuzione.** E' lo stesso buco di `LANT-42`: **sul corpus la ricevuta nomina `L4.1`/`L4.2`, in SDK da' `layers=None`.** ⇒ 🔑 **Per quella domanda serve la porta CLI o il ricalcolo con `run_validation_gate`, NON l'SDK** *(sugli 80 handoff il ricalcolo dava `L1.13` 68 · `L1.15` 40 · `L1.20` 2: quello e' il metodo che risponde)*.
