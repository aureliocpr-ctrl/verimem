...
```
tool MCP        hippo_skills_topology
test che nominano il tool     1 file
test che nominano il modulo   4 file
```
...
|---|---|---|---|---|---|---|---|
| 1 | `verimem/skills_topology.py:15` `skills_topology` | funzione: Compute DAG-level topology stats. | `verimem/dashboard_overview.py:22`; `verimem/dashboard_overview.py:41`; `verimem/mcp_server.py:11351` (+2) | `tests/test_la_profondita_e_il_cammino_piu_lungo.py`; `tests/test_skill_lineage_metrics.py`; `tests/test_skills_topology.py` | - | NON MISURATO | - |
| 2 | `verimem/skills_topology.py:50` `skills_topology._profondita` | funzione: (nessun docstring) | `verimem/skills_topology.py:58`; `verimem/skills_topology.py:63` | nessuno | - | NON MISURATO | - |
