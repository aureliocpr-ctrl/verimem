# `verimem/l1_quality_detector.py` — L1.21
...
```
(1) SCATTA su      «The pipeline is flawless and bug-free.»           senza evidenza → ✅ warning
(2) NON scatta su  «The pipeline has two stages.»    → ✅ nessun warning
...
|---|---|---|---|---|---|---|---|
| 1 | `verimem/l1_quality_detector.py:53` `QualityClaimWarning` | classe: (nessun docstring) | `verimem/l1_quality_detector.py:76`; `verimem/l1_quality_detector.py:85`; `verimem/l1_quality_detector.py:95` | nessuno | - | NON MISURATO | - |
| 2 | `verimem/l1_quality_detector.py:58` `_has_evidence` | funzione: (nessun docstring) | `verimem/l1_quality_detector.py:83`; `verimem/truth_reconciliation.py:145`; `verimem/truth_reconciliation.py:147` | nessuno | - | **FUNZIONA COME PROMESSO** | eseguita, 1/11 statement del corpo scoperti (755 passed, EXIT=0) |
| 3 | `verimem/l1_quality_detector.py:74` `detect_unsupported_quality_claim` | funzione: Warn on an unsupported quality superlative; else None. | `verimem/anti_confab_gate.py:1672`; `verimem/anti_confab_gate.py:1673`; `verimem/l1_quality_detector.py:95` | `tests/test_l1_quality_detector.py` | - | **FUNZIONA COME PROMESSO** | banco famiglia: scatta su «The pipeline is flawless and bug-f…» senza evidenza, tace sulla prosa innocua; 755 passed, 6 xfailed, EXIT=0; modulo al 96.1% |
