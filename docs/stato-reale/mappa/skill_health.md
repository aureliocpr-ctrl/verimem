...
```
tool MCP        hippo_skill_health
test che nominano il tool     1 file
test che nominano il modulo   6 file
```
...
|---|---|---|---|---|---|---|---|
| 1 | `verimem/skill_health.py:35` `_days_since` | funzione: Convert a Unix-epoch timestamp to days-since-now. Returns | `verimem/skill_health.py:127` | nessuno | - | NON MISURATO | - |
| 2 | `verimem/skill_health.py:45` `_suggest_action` | funzione: Apply the policy. Returns (action, reasoning). | `verimem/skill_health.py:137` | nessuno | - | NON MISURATO | - |
| 3 | `verimem/skill_health.py:104` `skill_health` | funzione: Compute the health diagnostic for a single skill. | `verimem/apply_recommendations.py:1`; `verimem/apply_recommendations.py:12`; `verimem/apply_recommendations.py:29` (+18) | `tests/test_apply_recommendations.py`; `tests/test_mcp_server.py`; `tests/test_recommend_actions.py` (+3) | - | NON MISURATO | - |
