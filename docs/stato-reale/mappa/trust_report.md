...
Ogni riga porta il proprio livello scritto per esteso. Su `trust_report.py` **nessuna
...
# Mappa di `verimem/trust_report.py` — 4 righe (bozza da confermare leggendo ed eseguendo)

| # | funzione (`file:riga`) | cosa promette | chiamata da | test che la esercita | claim README | verdetto | prova |
|---|---|---|---|---|---|---|---|
| 1 | `verimem/trust_report.py:43` `_fact_evidence` | funzione: One fact's full custody record. Best-effort: a history/dispute lookup | `verimem/trust_report.py:311` | `tests/test_freschezza_non_nasce_da_un_tocco.py` | - | **FUNZIONA COME PROMESSO** *(al livello: i test passano)* | batch 08/09 21:15, 48 file eseguiti in un colpo: **2315 passed, 6 skipped, 43 xfailed, 25 warnings in 1421.89s (0:23:41)**, EXIT=0, zero rossi. ⚠️ **Livello dichiarato**: questo prova che i test che la nominano passano, NON che la funzione faccia ciò che il suo docstring promette — la seconda cosa richiede casi scelti sulla promessa, e per questa riga non l'ho fatta |
| 2 | `verimem/trust_report.py:148` `_ce_relevance_floor` | funzione: (nessun docstring) | `verimem/trust_report.py:159`; `verimem/trust_report.py:184` | `tests/test_verimem_env_thresholds.py` | - | **FUNZIONA COME PROMESSO** *(al livello: i test passano)* | batch 08/09 21:15, 48 file eseguiti in un colpo: **2315 passed, 6 skipped, 43 xfailed, 25 warnings in 1421.89s (0:23:41)**, EXIT=0, zero rossi. ⚠️ **Livello dichiarato**: questo prova che i test che la nominano passano, NON che la funzione faccia ciò che il suo docstring promette — la seconda cosa richiede casi scelti sulla promessa, e per questa riga non l'ho fatta |
| 3 | `verimem/trust_report.py:157` `_apply_ce_gate` | funzione: Drop hits the cross-encoder scores as NOT relevant to ``query`` (CE logit | `verimem/trust_report.py:149`; `verimem/trust_report.py:249` | `tests/test_verimem_env_thresholds.py` | - | **FUNZIONA COME PROMESSO** *(al livello: i test passano)* | batch 08/09 21:15, 48 file eseguiti in un colpo: **2315 passed, 6 skipped, 43 xfailed, 25 warnings in 1421.89s (0:23:41)**, EXIT=0, zero rossi. ⚠️ **Livello dichiarato**: questo prova che i test che la nominano passano, NON che la funzione faccia ciò che il suo docstring promette — la seconda cosa richiede casi scelti sulla promessa, e per questa riga non l'ho fatta |
| 4 | `verimem/trust_report.py:189` `build_trust_report` | funzione: Build the evidence dossier for ``query``: the retrieved facts with their | `verimem/cli.py:2316`; `verimem/client.py:2275`; `verimem/client.py:2280` (+5) | `tests/test_cli_trust_declares_what_ran.py`; `tests/test_il_dossier_si_asteneva_per_la_ragione_sbagliata.py`; `tests/test_il_proxy_pigro_nasconde_il_provider_mancante.py` (+5) | - | **FUNZIONA COME PROMESSO** *(al livello: i test passano)* | batch 08/09 21:15, 48 file eseguiti in un colpo: **2315 passed, 6 skipped, 43 xfailed, 25 warnings in 1421.89s (0:23:41)**, EXIT=0, zero rossi. ⚠️ **Livello dichiarato**: questo prova che i test che la nominano passano, NON che la funzione faccia ciò che il suo docstring promette — la seconda cosa richiede casi scelti sulla promessa, e per questa riga non l'ho fatta |
