...
| 79 | `facts_forget` | 3539 | comando Typer `forget` | `tests/test_un_ripristino_non_avvenuto_non_dice_successo.py::test_anche_un_OBLIO_abortito_non_dice_successo` (T58, 10/09) | FUNZIONA COME PROMESSO — `PYTHONPATH=<repo> python -m pytest -q -p no:randomly tests/test_un_ripristino_non_avvenuto_non_dice_successo.py` -> 5 passed EXIT=0; contro origin/main 32273665 (pre-cura, worktree separato) 3 failed 2 passed EXIT=1. ⚠️ prima di T58 usciva **0** se annullato: `facts forget X && echo cancellato` scriveva che il fatto era sparito mentre era ancora nello store (due punti: 3625, 3659) |
...
| 85 | `facts_restore` | 4039 | comando Typer `restore` | `tests/test_un_ripristino_non_avvenuto_non_dice_successo.py` (4 casi, T58, 10/09) | FUNZIONA COME PROMESSO — `PYTHONPATH=<repo> python -m pytest -q -p no:randomly tests/test_un_ripristino_non_avvenuto_non_dice_successo.py` -> 5 passed EXIT=0; contro origin/main 32273665 (pre-cura, worktree separato) 3 failed 2 passed EXIT=1. ⚠️ prima di T58 un ripristino annullato o interrotto usciva **0** (riga 4061); gli altri due rami uscivano gia' 1 |
...
| 90 | `facts_anti_confab_apply` | 4300 | comando Typer `anti-confab-apply` | **nessun caso nel banco** — ⚠️ LIMITE DICHIARATO | NON MISURATO. La riga dell'aborto (4340) e' CURATA da T58 insieme alle altre tre, ma nessun test la esercita: chi la tocca aggiunga il caso a `tests/test_un_ripristino_non_avvenuto_non_dice_successo.py` |
