...
## Le voci di «Not solved yet» del CHANGELOG — CHI LE POSSIEDE

*La lista pubblica della 0.7.7 si chiude con: «**Owners and tickets are tracked in
`docs/stato-reale/GRAVITA-DIFETTI.md`**». **Questo è quel posto**, e fino a oggi
(07/09, `2b82497f`) i nomi non c'erano: chi seguiva il rimando trovava i ticket ma
non un responsabile, e per due voci su sei non trovava nemmeno il ticket.*
...
**① La voce di punta del CHANGELOG annuncia una capacità REVERTITA.**
`CHANGELOG.md:32` (nella sezione delle cose **fatte**, non fra i limiti) dice
*«Now they **coexist** and the receipt says why (`L3-fonti-distinte`)»*, e
`README.md:339` lo ripete. Il testo è di `f552e6eb` (06/09 **14:45**); la cura è
stata revertita da `05c26887` (06/09 **15:31**, «rompeva quattro test, torna nella
0.7.8»), che **è antenato del tip**. Nel codice di oggi:

```
grep -rn "L3-fonti-distinte" verimem/
  verimem/cli.py:4661:   # dalla coesistenza fra fonti distinte (`L3-fonti-distinte`), che la
```

🔬 **E la prova non si ferma al `grep`** — ho ripreso **il test che gli autori
stessi avevano scritto per quella capacità** (`tests/test_due_fonti_dichiarate_non_si_ritirano.py`,
287 righe, 11 test, rimosso dal revert) e l'ho eseguito **sul codice del tip**:

```
python -m pytest <il test recuperato da 05c26887^> -x -q      EXIT=2
  ImportError: cannot import name 'due_fonti_dichiarate_e_diverse'
  from 'verimem.supersession_policy'
  1 error in 32.26s
```

⚠️ **Classifico il rosso prima di spiegarlo**: **non** è un fallimento di
comportamento, è un **ImportError** — la funzione non esiste più. Per la tesi è
evidenza **più forte**, non più debole: non è che la coesistenza dia un esito
diverso, è che **non c'è il codice che la fa**. *(Il test non richiede il
giudice: importa solo `anti_confab_gate` e `supersession_policy`, quindi nessuno
slot occupato e nessun modello caricato.)*

**una sola occorrenza in tutto `verimem/`, dentro un COMMENTO** (l'altra copia è
un commento in `tests/test_all_write_channels_judge_a_source.py:240`). ⇒ Nessun
...
**③ Il titolo di D-1 nel CHANGELOG descrive il rischio della CURA, non il difetto
aperto.** Dice *«a fact reported by a third party can be mistaken for a
contradiction»* — che è cosa faceva la cura revertita (fermava anche i veri
composti). Oggi, senza cura, il prodotto **non** scambia i fatti di terzi: fa
l'opposto, **lascia passare una self-claim in coda a una frase vera**, perché il
...
*Alle 13:40 il lead ha nominato i pezzi di T26 in modo suo, e **due sistemi di
nomi sullo stesso ticket sono esattamente il difetto che questa sezione cura**.
La mappa, così CHANGELOG e scheda non divergono:*
...
| **T29** — il giudizio mancante non si sana mai | **T29** | **P1**, sale a P0 se il recall non li distingue |

📌 **Il mio vecchio «T26b» (il regresso da ~60 s) NON è il T26b del lead**: era il
costo dell'attesa, ed è **chiuso** dal revert di @ws5 delle 13:32. *Un numero
riusato per due cose diverse è un numero che mente: qui è scritto quale è quale.*

### Le due righe che mancavano
...
· **@ws6, i nove file alla radice**: il suo è il lato *store* dello stesso problema —
...
· **@ws5** l'aveva **sfiorato** il 05/09 ritirando il ticket 8: *«il Quickstart del
