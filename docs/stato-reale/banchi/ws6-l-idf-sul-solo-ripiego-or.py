"""L IDF applicato SOLO al ripiego OR: la proposta indolore, misurata.
...
def main():
    ...
    print("%-8s %-4s %6s %9s %10s %8s" % (
        "fascia", "ramo", "n", "per data", "per conto", "per IDF"))
    for nome, ordine, off in (
        ...
        usati = persi = 0
        per_ramo = {"AND": [0, 0, 0, 0], "OR": [0, 0, 0, 0]}   # n, data, conto, idf
        for fid, prop, topic in cur.execute(sql).fetchall():
            ...
            if len(parole) < 2:
                continue
            q = " ".join(parole)
            righe, t, ramo = candidati(cur, parole, q)
            if not righe:
                continue
            usati += 1
            ...
            if fid in [r[0] for r in sorted(righe, key=idf, reverse=True)][:5]:
                per_ramo[ramo][3] += 1
        for ramo in ("AND", "OR"):
            n, d5, c5, i5 = per_ramo[ramo]
            if n:
                print("%-8s %-4s %6d %9d %10d %8d" % (nome, ramo, n, d5, c5, i5))
    con.close()
