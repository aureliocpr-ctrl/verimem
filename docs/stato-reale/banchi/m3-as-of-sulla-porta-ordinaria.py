...
def main() -> int:
    ...
    if rifiutato:
        ...
        return 0
    #: ⬇️ L'ESITO CHE LA CURA DEL 04/09 DEVE PRODURRE, ed e' il PRIMO ramo
    #: perche' e' quello atteso: entrambe le porte ordinarie danno lo STESSO
    #: insieme del tool dedicato, e la risposta DICHIARA il filtro applicato.
    #: Il banco resta falsificabile: togliete la cura e torna 🔴, perche' con
    #: `as_of` ignorato `id_con` ridiventa uguale a `id_senza`.
    if id_con == id_vero and id_s_con == id_vero and not id_con:
        _eco = {"as_of", "as_of_scartati"} <= _kc
        print("  🟢 as_of E' APPLICATO SU ENTRAMBE LE PORTE, e dichiarato.")
        print(f"     facts_recall: {len(id_senza)} senza -> {len(id_con)} con")
        print(f"     facts_search: {len(id_s_senza)} senza -> {len(id_s_con)} con")
        print(f"     il tool dedicato, stesso istante: {len(id_vero)}")
        print(f"     la risposta dichiara il filtro (as_of + as_of_scartati)? {_eco}")
        if not _eco:
            print("  🔴 …ma la risposta NON lo dichiara: un filtro applicato in")
            print("     silenzio e' il difetto di partenza in un'altra forma.")
            return 1
        return 0
    if id_con == id_senza:
