# -*- coding: utf-8 -*-
"""COPIA di `banco-osservatore-il-tasso.py` con UNA riga cambiata, per rieseguirlo.

Il banco originale (di un'altra istanza) fissa il proprio store accanto a se',
in `store_largo`, che nel repo di lavoro E' POPOLATO (2,8 MB) mentre il suo
docstring dichiara «corpus VUOTO». Qui lo store si puo' passare da fuori con
`WS6_STORE`, e questo permette due cose che sull'originale non si potevano fare:

  - eseguirlo nella condizione che DICHIARA (una tempdir vuota);
  - eseguirlo sulla condizione OPPOSTA (una copia dello store popolato) e
    vedere se il risultato cambia — senza toccare l'originale di nessuno.

Esito del 2026-09-01 (documento 68): le due condizioni danno lo STESSO risultato
in ogni cifra (26/48 ammesse, 9/48 divergenze), quindi lo stato dello store non
sposta la misura. La differenza rispetto al 25/08 pubblicato nel README
(25/48 e 8/48) non e' quindi la condizione iniziale.

⚠️ Il resto del file e' dell'autore originale e non e' stato toccato.
"""
import os, sys, io, contextlib, re

BANCO = os.path.dirname(os.path.abspath(__file__))
STORE = os.environ.get("WS6_STORE") or os.path.join(BANCO, "store_largo")
