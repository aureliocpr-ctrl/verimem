...
| 7 | «Trust odometer `m.trust_stats()`» | ✅ | verificato live: ritorna `{ledger:{admitted/quarantined/rejected/abstained}, by_layer, daily, store}` |
...
| 11 | «each tenant an isolated store, derived from API key» | ✅ core + difetti chiusi | adversarial opus: **PASS** sull'invariante (DB-per-tenant, tenant da key). I 6 difetti MED/LOW sono ora **6/6 gestiti**: local_tenant collision (`fca0e15`), `_TENANT_RE` trailing-dot (`\Z`), IPv6 Host parse (`c246522`) — chiusi TDD; **SSE `/v1/events/flow` DoS** → tail incrementale a byte-offset (`f221e87`, 7 test); **quota TOCTOU** → reserve-counter atomico (`3761b69`, 6 test incl. 40-thread race); host-spoofing via `curl` = mitigato dal bind loopback (scelta di design documentata, #10). Nessun HIGH residuo. |
...
| 13 | write-gate AUROC 0.971 (moat anti-confab) | ✅ proprietà / 🔬 numero-esatto | Ricevuta R10 REALE documentata (`EPISTEMIC_FAILURES_STUDY.md:375`: SNLI AUROC **0.971**, AURC 0.165, Youden 50; R11 wrong-source **0.992**). Il gate DI DEFAULT = judge LLM (il CE locale è opt-in `ENGRAM_GROUNDING_BACKEND=local`). Fresco 2026-07-16: gate-discrimination sul noise **SAME-TOPIC** (caso HARD — confab on-topic vs il dialogo che fonda la risposta giusta) = noise-reject **20/20=1.0**, clean-admit **20/20=1.0** (`writepath_moat_sametopic_opus_2026-07-16.json`). Corrobora la proprietà oggi. Il NUMERO AUROC esatto NON è ri-misurato oggi (serve judge opus su SNLI ~150 call → gated su OK budget). NB: la QA end-to-end di quel run è n=0 (config confab-set vuoto in same-topic), = non-dato non regressione. |
