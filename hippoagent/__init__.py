"""Deprecated alias for the `engram` package (formerly `hippoagent`).

The package was renamed from ``hippoagent`` to ``engram`` in v0.3.0
...
All accesses are forwarded to the corresponding ``engram`` module via
:py:meth:`sys.modules` pre-registration — both names point at the same
module object, so identity (``hippoagent.X is engram.X``), :func:`isinstance`,
...
your code to use ``from engram... import ...`` directly.

Implementation note (cycle #41 critic-orchestrator fix, job 614b682c67d39a7f):
the first attempt used a meta-path finder returning the engram spec —
CPython's ``_load_unlocked`` re-executed the engram module body and
...
``sys.modules['engram.X']`` before any user import resolves.
"""
...
warnings.warn(
    "The 'hippoagent' package has been renamed to 'engram'. "
    "Update your imports: `from engram... import ...`. "
    "This backward-compat shim will be removed in 3 months (~2026-08-13).",
    DeprecationWarning,
    stacklevel=2,
)

# Eager import of engram so __version__, top-level symbols are available.
import engram as _engram  # noqa: E402

__version__ = _engram.__version__


def _pre_populate_aliases() -> int:
    """Register every ``engram.X`` submodule under ``hippoagent.X`` in
    :data:`sys.modules`, pointing at the SAME object.

    Walking the engram package tree forces import of every submodule once;
    ...
    for modinfo in pkgutil.walk_packages(
        _engram.__path__, prefix="engram.",
    ):
        try:
            ...
            # those imports would also fail under the canonical `engram.X`
            ...
            continue
        alias = "hippoagent" + modinfo.name[len("engram"):]
