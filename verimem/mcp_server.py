    ...
    return [
        ...
        t.Tool(
            ...
            description=(
                ...
                "facts. ⚠️ SO DOES A SHORT ONE, AND THAT IS THE TRAP: "
                "results are ALWAYS ordered `created_at DESC` — never "
                "by relevance, which does not exist here — so a query "
                "that matches a lot returns THE NEWEST FACTS, not the "
                "best ones. Measured at this port on 2026-08-31: the "
                "one-letter query `l` filled the cap (200 of 200 rows, "
                "160289 characters — enough to overflow an MCP "
                "client's window), and `LIKE '%l%'` matches 16273 of "
                "~16.8k facts. ⇒ ON SHORT INPUT THIS PORT DOES NOT "
                "FILTER, and it does not abstain either: it is the "
                "OPPOSITE failure from the empty result a caller "
                "expects. The receipt says so — `ricerca.ordinati_per` "
                "— read it. Optional `topic` filter. Cycle #109 S4-A: "
