...
or run this stand-alone via `python -m engram.engram_syscall_mcp`.
...
Run: python -m engram.engram_syscall_mcp
"""
...
def tool_engram_invoke_recall(query: str, k: int = 5,
    ...
    """
    from engram.syscall_bridge import engram_invoke
...
def tool_engram_invoke_audit_tail(n: int = 20) -> list[dict]:
    """Read last N entries from engram syscall audit JSONL."""
    from engram.syscall_bridge import engram_audit_tail
...
def tool_engram_dashboard(format: str = "text", tail: int = 30) -> str:
    ...
    """
    from engram.dashboard_widget import collect_state, render_json, render_text
...
def main() -> int:
    ...
    print("[engram-syscall-mcp] MCP SDK available — server mode TBD by "
          ...
          "engram.engram_syscall_mcp.get_tool_definitions()",
          file=sys.stderr)
    print(json.dumps({
        "server": "engram-syscall-mcp",
        "version": "cycle-366",
