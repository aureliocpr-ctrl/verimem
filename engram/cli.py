...
# Wrapper Typer attorno a engram.consolidation.{auto_consolidate,
...
def dashboard(
    ...
    uvicorn.run("engram.dashboard:app", host=host, port=port, log_level="warning")
...
def facts_archive_narration(
    ...
    use_llm: bool = typer.Option(
        False, "--use-llm",
        help="Use the LLM extractor (engram.llm.get_llm, hosted) for higher-recall "
...
# orchestrator (engram.consolidation). Three sub-commands:
#
...
def consolidate_status() -> None:
    ...
    ``AUTO-CLUSTER-MASTER `` tag (see engram.consolidation).
    """
    sm = _facts_sm()
    with sm._connect() as conn:  # noqa: SLF001
        row = conn.execute(
