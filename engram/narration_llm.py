"""LLM-refined atomic extraction from narration (engram.narration_llm).

The rule-based ``engram.narration.extract_atomic_candidates`` only keeps clauses
...
``engram.openie``: a JSON-only contract, ONE retry on a malformed parse, and an
...
``resp.text`` the model output) — i.e. ``engram.llm.get_llm()``, which in HOSTED
...
def extract_atomic_facts(
    ...
    Anti-confab (mirrors ``engram.openie``): JSON-only contract, ONE retry on a
