"""Narration vs atomic knowledge (engram.narration).
...
it. This module is the deterministic, no-LLM FOUNDATION:
  ...
  - ``extract_atomic_candidates`` — split the prose into short clauses that carry
    ...
    rule-based first pass; an LLM refinement (engram.openie / sleep) can replace
...
def archive_and_extract_narration(
    ...
    an ``llm`` is given (``engram.llm.get_llm()``), the higher-recall LLM extractor
    (``engram.narration_llm.extract_atomic_facts``) is used instead.

    Safety contract (mirrors engram.admission_cleanup):
