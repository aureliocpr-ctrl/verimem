    ...
    return [
        t.Tool(
            ...
            description=(
                ...
                "sandbox (engram.sandbox.SandboxedShell) instead of the "
...
async def _call_tool_impl(name: str, arguments: dict[str, Any]) -> list[t.TextContent]:
    ...
    try:
        if name == "sandbox_exec":
            # Task #48 — thin wrapper over engram.sandbox.SandboxedShell.
        ...
        if name == "hippo_warmup_status":
            ...
            from engram import encode_service as _svc
        ...
        if name == "hippo_record_episode":
            ...
            from engram.syntax_pollution import sanitize_proposition as _sp
            fact_ids: list[str] = []
        ...
        if name == "hippo_trajectory_summary":
            from engram.trajectory import trajectory_from_json
            from engram.trajectory_render import trajectory_summary_line
            try:
        ...
        if name == "hippo_export_dot":
            from engram.graphviz_export import export_dot
        ...
        if name == "hippo_health_report":
            from engram.memory_health_report import generate_health_report
            eps, facts_all = [], []
            try:
                eps = a.memory.all(limit=10000)
                facts_all = a.semantic.list_facts(limit=10000, offset=0)
        ...
        if name == "hippo_hallucination_rate":
            from engram.hallucination_rate import hallucination_rate_at_k
        ...
        if name == "hippo_forward_chain":
            from engram.symbolic_inference import (
                forward_chain,
                parse_rule,
            )
            facts_all = []
        ...
        if name == "hippo_emerging_skills_register":
            ...
            from engram.skill_emergence_detector import (
        ...
        if name == "hippo_audit_summary":
            ...
            # `engram.audit_tail.audit_tail` which already exists and
        ...
        if name == "hippo_episodes_find_duplicates":
            from engram.episode_dedup import find_duplicate_groups
        ...
        if name == "hippo_episodes_dedup":
            from engram.episode_dedup import dedup_episodes
        ...
        if name == "hippo_skill_exposure_audit":
            from engram.skill_exposure_audit import (
        ...
        if name == "hippo_self_model_refresh":
            from engram.self_model import SelfModelStore
            from engram.self_model_refresh import (
                compute_diff,
                propose_refresh,
            )
            data_dir = (
        ...
        if name == "hippo_facts_find_polluted":
            from engram.syntax_pollution import scan_facts
            facts_all = []
            try:
                facts_all = a.semantic.list_facts(
        ...
        if name == "hippo_skills_derive_predicates_batch":
            from engram.predicate_derivation import (
        ...
        if name == "hippo_validate_claim":
            from engram.validate_claim import validate_claim as _vc
        ...
        if name == "hippo_remember":
            ...
            # cycle-#70 brittle prefix-cut with engram.syntax_pollution.
            ...
            from engram.syntax_pollution import sanitize_proposition
        ...
        if name == "hippo_dashboard_overview_v2":
            from engram.dashboard_overview_v2 import dashboard_overview_v2 as dashboard_overview
        ...
        if name == "hippo_decay_run":
            ...
            from engram.decay_job import SEC_PER_DAY, run_decay_pass
...
def main() -> None:
    ...
    (engram.preload) so it no longer blocks the JSON-RPC attach handshake —
