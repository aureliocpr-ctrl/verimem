...
#: all-MiniLM-L6-v2 (384 dim) and ``engram.embedding.serialize`` writes
...
#: SINGLE SOURCE OF TRUTH = engram._telemetry_prefixes.TELEMETRY_TOPIC_PREFIXES
...
def _topic_penalty_strength() -> float:
    """Strength of the off-topic recall penalty (engram.topic_priors). Default 0.0
...
def _apply_topic_penalty_to_sims(sims, facts, query_text):
    """Down-rank broadly-matching off-topic facts (engram.topic_priors) for task-style
